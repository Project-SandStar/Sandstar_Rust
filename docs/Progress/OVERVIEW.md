...
9. **Phase 13 — Dynamic slots.** ✅ Complete. Runtime tag addition per component without recompile. `DynSlotStore` (1559 LOC + 59 tests) with memory limits (64/comp, 10K total), atomic JSON persistence + 5s auto-save, REST `/api/tags/{comp_id}`, ROX `readTags`/`setTags`/`deleteTag` over WebSocket (`rest/rows.rs`), SOX `readComp` extension, Haystack filter integration (`filter::matches_with_tags`). 4 integration tests added 2026-04-18 proving WS round-trip + filter composition. Research: [research/19_DYNAMIC_SLOTS.md](../research/19_DYNAMIC_SLOTS.md).
