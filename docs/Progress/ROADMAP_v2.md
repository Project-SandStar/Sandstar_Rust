...
| Research coverage | **~99.75% weighted** (19 of 20 docs at 100%; Doc 04 at 95% — Xeto/Commit/Root ops deferred). Prior "72% weighted" figure pre-dated the 2026-04-18/19 audit. |
...
|-----|-------|-------|----------|-------------|
| 00 | Executive Summary | All | 100% | All migration scope covered. Test count in the research doc itself is out of date (document says ~820; actual ~2671 as of 2026-04-18). The document's claims about what would ship are all met or exceeded. |
| 01 | Engine Core Analysis | 0, 2, 7.0 | 100% | Surpassed: PID, sequencer, components, priority arrays |
| 02 | Hardware Drivers | 3D, 7.0c | 100% | All driver targets shipped. Architectural divergence from the research doc: GPIO uses sysfs (stable, present on every BeagleBone Debian image) rather than the chardev approach the doc recommended; async I2C/UART was ruled unnecessary after profiling showed sync calls had no observable latency impact on 5-sec polls. |
| 03 | Haystack Type System | 2 | 100% | All required types shipped (Number, Str, Ref, Bool, Marker, DateTime, etc.). Architectural divergence: custom Rust implementation instead of a libhaystack dependency — fewer transitive deps, tighter control of MSRV + cross-compile story, smaller `.deb`. Interop verified via Zinc/Trio round-trip tests. |
| 04 | REST API (Axum) | 1 | 95% | All core Haystack ops live (about, read, eval, hisRead, nav, watchSub, watchPoll, watchUnsub, pointWrite, invokeAction). Surpassed the research doc with WebSocket push (Phase 8.0A), SCRAM auth (6.5b), rate limiting (5.7e), TLS (6.5a), and the Phase 12 driver-lifecycle surface. **Gap**: Xeto / Commit / Root ops are not implemented — low priority (no observed caller); would be a discrete piece of work if a specific integrator asked. |
| 05 | Zinc I/O Encoding | 3H | 100% | Full Zinc 3.0 encoder + decoder shipped (`rest/zinc_format.rs`), round-trip-tested against real grids. Architectural divergence: custom Rust implementation instead of a libhaystack-backed parser — same reasoning as Doc 03 (dep minimisation, cross-compile, size). |
| 06 | Sedona FFI Strategy | 4, 6.0 | 100% | All VM-to-host surfaces implemented. Architectural divergence: the research doc planned 29 C FFI functions to bridge Sedona VM ↔ Rust host; Sandstar replaced the entire FFI with an in-process Rust-native bridge (`bridge.rs` + Sedona VM ported to pure Rust in Phase 11.0). Zero C/FFI in the final image — the doc's FFI enumeration is structurally moot; equivalent functionality exists natively. |
| 07 | IPC Bridge | 3A | 100% | All CLI↔engine commands shipped (13 IPC ops, 19 CLI tests). Architectural divergence: research doc proposed POSIX IPC (shm + semaphores); Sandstar ships typed `bincode` length-prefixed frames over Unix domain socket (Linux) / TCP 127.0.0.1 (Windows) — portable, type-safe, no System V cleanup issues. Socket perms locked to 0660 (Phase 5.7f). |
| 08 | Memory Safety | All | 100% | All 13 C/C++ bug classes eliminated structurally |
| 09 | Dependency Mapping | All | 100% | All runtime deps resolved to workspace-pinned versions; zero C/C++ in the final binary. Architectural divergence: `libhaystack` replaced by custom types (Docs 03/05), Docker cross-compile replaced by Zig CC (Doc 10) — both reduce transitive deps and improve reproducibility of the `.deb`. |
| 10 | Build & Cross-compile | 3E | 100% | GitHub Actions CI/CD live (fmt + clippy + test on push/PR, ARM cross-compile on master). Zig CC wrappers (`C:\czb\zigcc-arm-cc.bat`) replace the research-doc Docker approach — smaller, faster, no container dependency. Previous 95% label reflected the pre-CI state. |
| 11 | Migration Roadmap | 0-5 | 100% | All 7 original migration phases complete. Surpassed the doc with post-migration additions: WebSocket push (8.0A), pure-Rust SOX (8.0A-SOX), config-driven control engine (10.0A-E), SCRAM-SHA-256 auth (6.5b), TLS (6.5a), Driver Framework v2 (Phase 12, all 8 sub-phases), Dynamic Slots (Phase 13), roxWarp clustering 9.0a+b+c (Phase 9). |
...
| 14 | Scalability Limits | 11.0, 5.6, 7.0c | 100% | All scalability fixes from the research doc implemented: `ComponentStore` (free-list allocator, u32 IDs, iterative tree walk to avoid recursion stack limits), `VmConfig` (configurable limits: 64 KB stack, 4096 components, 4 MB scode, Byte32 addressing). No observed scaling bottleneck on the BeagleBone target (150 channels, 16 polls, 35 components). |
...
| 16 | roxWarp Protocol | 9.0 | 100% | All in-repo work complete: 9.0a (Binary Trio diff), 9.0b (mDNS peer discovery), 9.0c (state reconciliation via LWW + version vectors). ~5000 LOC in `src/roxwarp/` + 200 LOC `mdns.rs`, 128+ unit tests, 3 integration tests proving two-node convergence (2026-04-18). 9.0d SkySpark Fantom pod is **scoped out of this workspace** — it's a separate Fantom-language project that would live in a SkySpark pod repo; when/if someone builds it, the existing roxWarp wire protocol is its interface. |
| 17 | Name Length Analysis | 11.0c | 100% | Recommended solution (Name Interning) fully implemented. `NameInternTable` (410 LOC + 17 tests) with 2-byte `NameId(u16)`; `ComponentTree.name_table` interns on add/rename and indexes on load. Phase 2 Haystack integration: `navName`/`dis` recognised in `rest/filter.rs`, `loader.rs` Zinc parsing, `dyn_slots.rs` known tags. Phase 3 roxWarp protocol extension: `nameTable: Vec<String>` in welcome message for bulk transfer (`handler.rs:51-52`). 31-char Sedona-compat name validation enforced across REST / RoWS / SOX add/rename / editor JS (2026-04-04). Previous 60% label was stale — every migration-phase item from §8 of the research doc is live. |
...
**Roadmap audit 2026-04-18 / 2026-04-19:** Prompted by two accidental findings that week (Phase 9.0a had ~5000 LOC hiding behind a "Not started" label; Phase 13 was 5/7 labeled not-started but actually 7/7 shipped). A systematic pass through every row in this document flipped six stale labels — Phase 12.0 master row, docs 16 + 18 coverage %, Phase 8.0Bf, Phase 6.0 master row, Phase 14.0A sub-task rows — and refreshed the Metrics Snapshot. A follow-up pass on 2026-04-19 walked every <100% research-doc-coverage row: Doc 17 promoted 60% → 100% (name interning fully shipped, labels were stale), Doc 00 + Doc 10 label fixes (stale test count / stale "no CI/CD" claim), and Docs 02/03/05/06/07/09/11/14/16 re-framed as deliberate architectural divergences (custom Zinc/Haystack/filter, in-process bridge replacing 29-fn C FFI, Zig CC replacing Docker, etc.) rather than implying pending work. Only Doc 04 retains a <100% label: Xeto/Commit/Root Haystack ops are genuinely not implemented (low priority, no observed caller). No code changes were made in either pass; both only reconciled documentation with shipping code. If a future reader finds another "Not started" or "<100%" claim that contradicts reality, the right fix is the same: grep the codebase, measure tests, update the label, commit.
